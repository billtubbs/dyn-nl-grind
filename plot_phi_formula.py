...
# Paper formula
phi_paper = np.where(ratio <= eps_threshold,
...
def phi_softplus(r, beta):
    ...
    sp = np.where(arg > 0,
                  arg + np.log1p(np.exp(-arg)),
                  np.log1p(np.exp(arg)))
    return np.sqrt(sp / beta)

fig, ax = plt.subplots(figsize=(8, 4.5))

ax.plot(ratio, phi_paper, color="C0", linewidth=2.0,
        label=r"$\varphi$ paper (eq. 3)", zorder=3)

colors = ["C1", "C2", "C3"]
for beta, color in zip(BETA_VALUES, colors):
    ax.plot(ratio, phi_softplus(ratio, beta), color=color,
            linewidth=1.4, linestyle="--",
            label=rf"softplus  $\beta={beta}$")

# Mark threshold
ax.axvline(eps_threshold, color="grey", linewidth=0.9, linestyle="--")
ax.text(eps_threshold + 0.04, 0.65,
...
ax.scatter([ratio_nop], [phi_nop], marker="o", s=50, color="k", zorder=5,
           label=f"NOP  (ratio={ratio_nop:.3f}, φ={phi_nop:.3f})")

ax.set_xlabel(r"$x_{ms} / x_{mw}$  (solids-to-water volume ratio)", fontsize=10)
