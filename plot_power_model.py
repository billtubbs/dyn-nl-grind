...
_sp  = np.where(_arg > 0, _arg + np.log1p(np.exp(-_arg)), np.log1p(np.exp(_arg)))
...
for phi_c, color in zip(PHI_C_VALUES, colors):
    ...
    ax.plot(y_JT, P, color=color, linewidth=1.6 if phi_c == PHI_C_NOP else 1.2,
            linestyle=ls, label=label)

    # Mark each curve's peak (always at J_TPmax)
    P_peak = mill_power(FILL_FRACTION_MAX_POWER, phi_c)
    ax.scatter([FILL_FRACTION_MAX_POWER], [P_peak], marker="^", s=35,
               color=color, zorder=4)

# NOP operating point
P_nop = mill_power(Y_JT_NOP, PHI_C_NOP)
ax.scatter([Y_JT_NOP], [P_nop], marker="o", s=60, color="k", zorder=5,
           label=f"NOP  ($J_T={Y_JT_NOP:.4f}$, $P={P_nop:.2f}$ MW)")

# Reference lines
ax.axvline(FILL_FRACTION_MAX_POWER, color="grey", linewidth=0.9, linestyle=":",
           label=rf"$J_{{TPmax}} = {FILL_FRACTION_MAX_POWER}$")
ax.axvline(Y_JT_NOP, color="k", linewidth=0.7, linestyle=":")

ax.set_xlabel(r"Charge fill fraction  $J_T = (x_{mw}+x_{ms}+x_{mr}+x_{mb})\,/\,v_{mill}$",
              fontsize=10)
